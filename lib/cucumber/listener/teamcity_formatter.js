var TeamCityFormatter = function(options) {
  ...
  self.handleBeforeScenarioEvent = function handleBeforeScenarioEvent(event, callback) {
    ...
    currentScenarioPath = scenario.getUri().replace(process.cwd(),'').slice(1);
  ...
  self.handleStepResultEvent = function handleStepResult(event, callback) {
    ...
    if (stepResult.isSuccessful()) {
      ...
      failedTestMessage = '##teamcity[testFailed name=\'%s\' ' + 
                          'message=\'FAILED at step: "' + escapeTeamcityString(step.getName()) + '" scenario: "' + escapeTeamcityString(currentScenarioPath) + '"\' ' +
                          'details=\''+ escapeTeamcityString(failureDescription) +'\']';
